...
from ska_sdp_spectral_line_imaging.diagnosis.utils import amp_vs_channel_plot


@mock.patch("ska_sdp_spectral_line_imaging.diagnosis.utils.create_plot")
def test_should_plot_single_stokes_i(create_plot_mock):
...
@mock.patch("ska_sdp_spectral_line_imaging.diagnosis.utils.create_plot")
def test_should_plot_all_stokes(create_plot_mock):
    visibilities = Mock(name="visibilities")
    visibilities.mean.return_value = np.array([[1, 2], [3, 4]])
    label = ["I", "Q", "U", "V"]
    amp_vs_channel_plot(visibilities, "title", "path", label=label)
